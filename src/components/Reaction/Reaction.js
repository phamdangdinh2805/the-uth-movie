...
const Reaction = ({ comment, setShowReaction, showReaction }) => {
  ...
  const handleReact = (type) => {
    ...
    if (comment?.reactions?.some((item) => item.userId === user.uid)) {
      const newReaction = comment.reactions.filter((item) => item.userId !== user.uid);

      const reactUserType = comment?.reactions.find((item) => item.userId === user.uid).type;
      ...
      {reactionGif.map((item) => (
        ...
        >
          ...
          {comment?.reactions.some((item) => item.userId === user?.uid) &&
            comment?.reactions.find((item) => item.userId === user?.uid).type === item.name && (
              <span className="dotted-blue" />
            )}
        </div>
      ))}
    </div>
