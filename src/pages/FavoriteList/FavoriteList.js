...
const FavoriteList = () => {
  ...
  const handleDeleteMovie = async (data) => {
    ...
    if (result) {
      ...
      const newFavoriteList = favoriteList.filter((item) => item.id !== result.id);
          ...
          {Array.isArray(favoriteList) ? (
            favoriteList.map((item) => (
              <div key={item.id} className="image-favorite">
                <Link to={`/details/${item.movie.media_type}/${item.movie.id}`}>
                  <MovieItem data={item.movie} />
                </Link>
                <div className="deltele-favorite" onClick={() => handleDeleteMovie(item)}>
